...
def compare_with_nist():
    ...
    ax1.set_xlabel(r'Energía [$\mathrm{MeV}$]', fontsize=12, labelpad=10)
    ax1.set_ylabel(r"Rango [$\mathrm{g/cm^2}$]", fontsize=12, labelpad=10)
    ax1.set_xscale('log')
    ax1.set_yscale('log')
    ...
    ax2 = ax1.twinx()
    ax2.plot(X, relative_error, label=r"Error relativo respecto a NIST ($\%$)", color='red', linewidth=1.5, alpha=0.7)
    ax2.set_ylabel(r"Error relativo respecto a NIST ($\%$)", color='red')
    ax2.tick_params(axis='y', labelcolor='red')

    plt.title(f"Comparación de Rango en {MATERIAL.upper()} con Datos NIST PSTAR")
    # Comprobar si el directorio existe, si no, crearlo
    if not os.path.exists(GRAPH_PATH):
        os.makedirs(GRAPH_PATH)
    plt.savefig(f"{GRAPH_PATH}fig_{MATERIAL}_comparing_range_with_NIST.png")
    fig.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        GRAPH_PATH = f"./data/Approximation/range/{MATERIAL}/graphs/"
        APPR_PATH = f"./data/Approximation/range/"
        NIST_PATH = f"./data/NIST/range/"
